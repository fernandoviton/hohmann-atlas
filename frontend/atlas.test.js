...
describe('transferArcPathD', () => {
  it('starts at origin position', () => {
    const d = transferArcPathD(20, 40, 0, Math.PI, 10);
    assert.ok(d.startsWith('M '));
    const [x, y] = d.slice(2).split(' L ')[0].split(' ').map(Number);
    // At angle 0, r=20: expect (20, 0)
    assert.ok(Math.abs(x - 20) < 0.01, `x should be ~20, got ${x}`);
    assert.ok(Math.abs(y) < 0.01, `y should be ~0, got ${y}`);
  });

  it('ends at destination position', () => {
    const endAngle = Math.PI;
    const d = transferArcPathD(20, 40, 0, endAngle, 10);
    const segments = d.split(' L ');
    const last = segments[segments.length - 1].split(' ').map(Number);
    // At angle π, r=40: expect (-40, ~0)
    assert.ok(Math.abs(last[0] - (-40)) < 0.01, `end x should be ~-40, got ${last[0]}`);
    assert.ok(Math.abs(last[1]) < 0.5, `end y should be ~0, got ${last[1]}`);
  });

  it('works for inbound transfer (r2 < r1)', () => {
    const d = transferArcPathD(40, 20, Math.PI / 2, -Math.PI / 2, 10);
    const first = d.slice(2).split(' L ')[0].split(' ').map(Number);
    // At angle π/2, r=40: expect (0, 40)
    assert.ok(Math.abs(first[0]) < 0.5, `start x should be ~0, got ${first[0]}`);
    assert.ok(Math.abs(first[1] - 40) < 0.01, `start y should be ~40, got ${first[1]}`);
  });
});
...
describe('transferPoint', () => {
  it('returns origin position at t=0', () => {
    const pt = transferPoint(20, 40, 0, Math.PI, 0);
    assert.ok(Math.abs(pt.x - 20) < 0.01);
    assert.ok(Math.abs(pt.y) < 0.01);
  });

  it('returns destination position at t=1', () => {
    const pt = transferPoint(20, 40, 0, Math.PI, 1);
    assert.ok(Math.abs(pt.x - (-40)) < 0.01, `x should be ~-40, got ${pt.x}`);
    assert.ok(Math.abs(pt.y) < 0.5, `y should be ~0, got ${pt.y}`);
  });

  it('midpoint radius is between r1 and r2', () => {
    const pt = transferPoint(20, 40, 0, Math.PI, 0.5);
    const r = Math.sqrt(pt.x * pt.x + pt.y * pt.y);
    assert.ok(r > 20 && r < 40, `midpoint radius ${r} should be between 20 and 40`);
  });

  it('works for inbound transfer', () => {
    const pt0 = transferPoint(40, 20, 0, Math.PI, 0);
    assert.ok(Math.abs(pt0.x - 40) < 0.01);
    const pt1 = transferPoint(40, 20, 0, Math.PI, 1);
    assert.ok(Math.abs(pt1.x - (-20)) < 0.01, `end x should be ~-20, got ${pt1.x}`);
  });
});
